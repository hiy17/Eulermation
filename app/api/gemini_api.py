...
def generate_euler_examples(euler_circuit):
    ...
    prompt_template = f"""
        ...
        ✅ IMPORTANT RULES:
        - Each story must reflect the **exact order of locations in the Euler circuit**, including returning to the starting point.
        - Do **not** change, skip, reorder, or improvise any part of the path.
        - Only include categories that logically fit the circuit. Skip those that don’t make sense.
        - Do **not** include any introductory text. Only return the formatted stories.

        ✅ FORMATTING:
        - Start each example with the category label like this: `Category: Story`

        ✅ STORY GUIDELINES:
        - Use a storytelling tone with a relatable character or context.
        - Be vivid, natural, and easy to visualize.
        - Each story must **clearly reflect** the traversal of **every edge once** and the return to the starting point.
...
if __name__ == '__main__':
    try:
        examples = generate_euler_examples("A->B->C->->D->A")
